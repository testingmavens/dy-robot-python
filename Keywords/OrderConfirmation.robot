...
Check Order Confirmation page
    Wait Until Page Contains Element    ${OrderConfirmation_Page}    30s    User is not redirected on Confirmation page after validated Checkout
    #Order Number
...
Verify Order Confirmation details
    Run Keyword And Warn On Failure    Element Should Contain    ${confirm_email}             ${guest_valid}    timeout=3s
    Run Keyword And Warn On Failure    Element Should Contain    ${confirm_shipping}          ${shipping_method_overnight}    timeout=3s
    Run Keyword And Warn On Failure    Element Should Contain    ${confirm_first_name}        ${FIRST_NAME}    timeout=3s
    Run Keyword And Warn On Failure    Element Should Contain    ${confirm_address1}          ${ADDRESS}    timeout=3s
    Run Keyword And Warn On Failure    Element Should Contain    ${confirm_address2}          ${ADDRESS2}    timeout=3s
    Run Keyword And Warn On Failure    Element Should Contain    ${confirm_postal}            ${ZIP}         timeout=3s
    Run Keyword And Warn On Failure    Element Should Contain    ${confirm_phone}             ${PHONE}    timeout=3s
    Run Keyword And Warn On Failure    Element Should Contain    ${confirm_order_total_top}   ${sub_total_cart}    timeout=3s
    Run Keyword And Warn On Failure    Should Be Equal As Strings    first    second
...
Verify that Create an Account section is NOT displayed for Registered Order Confirmation page
    ...
    Run Keyword And Continue On Failure    Element Should Not Be Visible    ${oc_create_acc_pwd_l}
...
Click Create Account button from Order Confirmation page
    CommonWeb.Check and Click    ${oc_create_acc_submit_l}
...
Check the validation messages for invalid emails on Create an Account form within Order Confirmation step
    ...
    FOR    ${value}    IN    @{lista}
        ...
        Click Create Account button from Order Confirmation page
        ${err_visible}=    Run Keyword And Return Status    Wait Until Element Is Visible    ${create_acc_err_l}
        IF    ${err_visible}
...
Check that the account was created for correct guest email
    Run Keyword And Warn On Failure    Wait Until Element Is Visible    ${my_acc_email_lbl_l}
    Run Keyword And Warn On Failure    Element Should Contain   ${my_acc_email_elm_l}    ${guest_valid}

Go to Order History from My Account
    CommonWeb.Check and Click    ${my_acc_order_history_l}
...
Order Confirmation PDP page is displayed
    Wait Until Page Contains Element    ${OrderConfirmation_Page}    30s    User is not redirected on Confirmation page after validated Checkout
    #Order Number
    Wait Until Page Contains Element    ${confirm_order_number}    10s    Order Number seems not displayed in OrderConfirmation page
    Scroll To Element    ${confirm_order_number}
...
Order Confirmation page is displayed for eu
    Run Keyword And Warn On Failure     Wait Until Element Is Visible  ${order_confirmation_header_eu}  10s  Order Confirmation Header is not displayed
